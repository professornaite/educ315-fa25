...
The fundamental property of sets is that elements are either members or non-members -- there is no in-between. The symbol $\in$ (epsilon) is used to denote membership. In this context, it means: "is an element of," "is a member of," or simply "is in." To indicate non-membership, we use $\notin$. For example, for the same set $A = \{1, 3, 5\}$, we write $6 \notin A$. 
:::
...
The set of **rational** numbers, $\mathbb{Q} = \{\dfrac{a}{b}, a \in \mathbb{Z}, b \in \mathbb{Z}, b = 0\}$
...
# What do we mean by zero? What do we mean by infinity?
